  ...
  if (platform.packer === 'rpmbuild') {
    return new Promise((resolve, reject) => {
      ...
      cp.on('exit', function(code) {
        ...
        const dst = path.join(process.cwd(), (isBeta ? path.join('@builds', 'rpm', 'beta') : path.join('@builds', 'rpm', 'stable')), `${platform.name}-${product.name}-${version.replace('b', '-beta')}-1.${proc.arch}.rpm`);
