...
(() => {
  ...
  if (fs.promises !== undefined) {
    ...
    fs.promises.readFile = async function readFile(path_, options_) {
      if (!insideSnapshot(path_)) {
...
(() => {
  ...
  process.dlopen = function dlopen() {
    ...
    if (insideSnapshot(modulePath)) {
      ...
      const tmpFolder = process.env.ih_temp_dir ? path.join(process.env.ih_temp_dir, 'pkg', hash) : path.join(tmpdir(), 'pkg', hash);
